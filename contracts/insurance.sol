...
contract Insurance {
    ...
    function fileClaim(uint256 amount) public{
        ...
        require(amount <= policies[msg.sender], "Claim amount cannot be more than the policy");
        claims[msg.sender] += amount;
    }

    function approveClaim(address policyholder) public {
        require(msg.sender == owner, "Only the owner can approve the Claims.");
    ...
    function destroy() public {
        require(msg.sender == owner, "Only the woner can destroy the contract.");
        selfdestruct(owner);
    }
